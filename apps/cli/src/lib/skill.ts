...
name: notekit
description: Use the NoteKit MCP tools to read, search, and write the user's notes, tasks, and secrets in their git-backed (often end-to-end-encrypted) vault. Trigger whenever the user asks to find/create/update a note or task, capture a decision, track work, or look something up in "my notes / my vault / NoteKit".
---

# NoteKit

NoteKit is a git-backed, end-to-end-encryptable personal vault of Markdown
**notes**, kanban **tasks**, **links**, and **secrets**. You interact with it
through the \`notekit_*\` / \`notes_*\` / \`tasks_*\` MCP tools (this server).
...
- **tasks** — actionable work items with a status (todo/in_progress/blocked/done) and priority. Use for "track / remind / TODO" requests.
...
## Core workflow
1. **Search before you create.** Call \`notes_search\` (or \`tasks_list\`) first so you update the right item instead of making a duplicate.
2. **Read with the path.** \`notes_read\` returns frontmatter + body. Use paths returned by search.
3. **Create/update** with \`notes_create\` / \`notes_update\` / \`tasks_create\` / \`tasks_update\`. Keep the title meaningful; put structured fields (tags, status, priority, assignee) in their dedicated args, not buried in the body.
4. **Move work** by updating a task's \`status\` ("mark X done" → status \`done\`).
...
## Conventions
- Notes/tasks are Markdown with YAML frontmatter (\`title\`, \`tags\`, \`status\`, \`priority\`, timestamps). The tools own the frontmatter — pass fields as args.
...
## Don'ts
- Don't fabricate note/task contents — read first.
...
(\`notekit note new/list/read\`, \`notekit task ...\`, \`notekit vault unlock\`).
